...
- **실시간 통합 검색**: 도서명, 저자, 출판사, 본문 내용을 기준으로 실시간 도서 검색을 제공합니다.
- **반응형 도서 카드**: 등록된 모든 도서를 그리드 뷰 형태로 나열하여 직관적인 조회가 가능합니다.

### 3. 상세 정보 조회 및 추천 (`BookDetail`)
- **상세 데이터 시각화**: 도서의 작가, 출판사, 본문 내용과 함께 등록일 및 최종 수정일을 함께 보여줍니다.
- **도서 추천**: 마음에 드는 도서에 '추천하기'를 눌러 실시간으로 추천수(좋아요)를 올릴 수 있습니다.
...
| **POST** | `/books` | 신규 도서 등록 | `title`, `author`, `publisher`, `content`, `coverImageUrl`, `likeCount`, `createdAt`, `updatedAt` |
| **PATCH** | `/books/:id` | 도서 정보 수정 | `title`, `author`, `publisher`, `content`, `updatedAt` |
...
### 사전 준비 사항
- 이미지 생성을 위해서는 **OpenAI API Key**가 필요합니다.
- API Key는 보안을 위해 별도의 파일 저장이나 환경 변수가 아닌, 웹 화면 UI에서 직접 입력하여 메모리에만 올려 사용하도록 되어 있습니다.
